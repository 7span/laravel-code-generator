...
class RouteHelper
{
    public static function makeRouteFiles($modelName, $methods, $generatedFilesPath, $adminCrud)
    {
        ...
        if (count($methods) == 5) {
            $route = "Route::apiResource('" . Str::plural(strtolower($modelName)) . "', " . 'V1' . '\\' . ucfirst($modelName) . 'Controller::class);';
        } else {
            $route = "Route::apiResource('" . Str::plural(strtolower($modelName)) . "', " . 'V1' . '\\' . ucfirst($modelName) . "Controller::class)->only(['" . implode("', '", $methods) . "']);";
        }
        ...
        if ($adminCrud == '1') {
            ...
            $route = "Route::apiResource('" .Str::plural(strtolower($modelName)) . "', " . 'Admin' . '\\' . ucfirst($modelName) . 'Controller::class);';
