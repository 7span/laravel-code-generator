...
document
    .getElementById("migration_fields")
...
document.getElementById("seeder").addEventListener("click", function () {
    ...
    for (var i = 0; i < input.length; i++) {
        var a = input[i];
        seederArray.push(a.name.replace("table_fields[", "").replace("]", ""));
    }

    if (seederArray.length == 0) {
        document.getElementById("seeder").checked = false;
        $("#seederErr")
            .html("Please add at least one table field.")
            .css("color", "red");
    } else {
        if (checkbox.checked) {
...
$(document).on("change", ".relation_ship", function (e) {
    ...
    if (
        ...
        relationShipType == "hasOneThrough" ||
        relationShipType == "hasManyThrough"
    ) {
        $(this)
            .closest("tr")
            .find(".relation_another_model")
            .prop("disabled", false);
    } else {
        $(this)
            .closest("tr")
...
$("#makeFileForm").on("submit", function (e) {
    ...
    $.ajax({
        ...
        error: function (response) {
            ...
            response.responseJSON.hasOwnProperty("model_name")
                ? $(".modelNameError").text(response.responseJSON.model_name[0])
                : "";
            response.responseJSON.hasOwnProperty("method")
